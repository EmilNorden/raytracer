...
impl MeshInstance {
    ...
    pub fn calculate_normal_matrix_and_orientation(transform: &Matrix4<f32>) -> (nalgebra::Matrix3<f32>, f32) {
        ...
        (normal_matrix, orientation_sign)
    }
...
impl Intersectable for MeshInstance {
    ...
    fn intersect(&self, ray: &Ray, t_min: f32, t_max: f32) -> Option<Intersection> {
        ...
        self.data.intersect(&object_space_ray, t_min, t_max).map(|x| {
            let (normal, tangent) = transform_normal_and_tangent(&self.normal_matrix, self.orientation_sign, x.normal, x.tangent);
            Intersection {
                dist: x.dist,
